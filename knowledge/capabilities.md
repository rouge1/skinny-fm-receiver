...
| Radio, Tuner and RDS boxes in the Receive tab | ✅ | Each folds on a chevron at the right of its title (or a click on the title), remembered in the settings (`folded`); a folded Radio box keeps its Center and Center on tuner in sight (`widgets.Card`, `test_gui.py` part 3). The boxes are laid out on a grid (`widgets.Form`), not a `QFormLayout`, which in Qt 5 keeps the spacing of hidden rows: the Sweep box had a 30 px gap where its hidden rows were. Radio: Center + Center on tuner, Tuner range, IQ bandwidth. Tuner: the tuner, its ▲/▼ and the Step knob, then the channel filter. RDS: station, standard, stereo and snap, Clear RDS, signal, audio, then the RDS details (moved in from beside the MPX view, which now has the bottom right to itself). Seen on the BB60D in Slate and Walnut. |
...
| Sweep and Tuner boxes in the Sweep tab | ✅ | Sweep: band, bounds, RBW (or the hopping rows), Pause, the plan, station threshold. Tuner: the tuner and Listen. Seen on the BB60D. |
