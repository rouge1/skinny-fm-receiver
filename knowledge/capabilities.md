...
| Dashed line at the Center, on the spectrum | ✅ | Since 2026-09-22 the waterfall is left clear of it and of the tuner's marker. Yellow (the theme's `warn`) in Slate and Reading Room; verdigris in Walnut, whose amber was too near its orange tuner marker and tan trace (`widgets.MARKERS`). Seen on the BB60D in all three themes. |
...
| Waterfall palettes from each theme's colours | ✅ | Slate *Ice and ember* (option B since 2026-09-22: pale ice, then orange for the strongest), Reading Room *Ink on paper*, Walnut *Dial glow*, blended in Oklab from the theme's tokens (`widgets.WATERFALL`). Other candidates were rendered for choosing; see [roadmap.md](roadmap.md) item 1. |
