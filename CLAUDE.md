...
- Code: `tools/fm_receiver/`. `app.py` is the window, `engine.py` the
  ...
  another computer over ssh), `rtl433.py` the rtl_433 chain, for the
  rtl_433 tab and Receive's rtl_433 card (the radio's samples piped into
  rtl_433 processes, their JSON read back;
